...
class AppDownload: NSObject {
    ...
    func cancelDownload() {
        Logger.shared.log(message: "AppDownload.cancelDownload: User cancelled the download", type: .info)
        downloadTask?.cancel()
        ...
        do {
            ...
            if let appDirectory = contents.first(where: { $0.pathExtension == "app" }) {
                ...
                if fileManager.fileExists(atPath: codeSignatureDirectory.path) {
                    ...
                    Logger.shared.log(message: "Removed _CodeSignature directory")
                }
            ...
            Logger.shared.log(message: "\(error)")
            if fileManager.fileExists(atPath: destinationURL.path) {
                try? fileManager.removeItem(at: destinationURL)
            }
            cancelDownload()
...
func handleIPAFile(destinationURL: URL, uuid: String, dl: AppDownload) throws {
    ...
    if let error = functionError {
        DispatchQueue.main.async {
            Logger.shared.log(message: error.localizedDescription, type: .error)
        }
        throw error
    } else {
        DispatchQueue.main.async {
            Logger.shared.log(message: "Done!", type: .success)
